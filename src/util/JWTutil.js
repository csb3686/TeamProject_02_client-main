...
const beforeReq = (config)=>{
    ...
    if(!loginUser || !loginUser.userid){
        // 현재 로그인 정보가 없다면 에러메세지를 갖고 jaxios 로 요청한 곳으로 돌아갑니다
        alert('로그인이 필요한 서비스 입니다.')
        return Promise.reject(
            {response:{data:{error:"REQUIRE_LOGIN"}}}
            // REQUIRE_LOGIN은 react에서 result.data.error를 console에 출력하면 확인이 가능합니다
        )
    }
...
const beforeRes = async (res)=>{
    ...
    // 토큰에러에 대한 응답인지를 체크해서 토큰에러라면 토큰을 갱신하고 현재요청을 재요청합니다
    ...
    if(data && data.error == 'ERROR_ACCESS_TOKEN'){
        // 토큰 기간 만료된 경우
        const result = await axios.get(`/api/member/refresh/${loginUser.refreshToken}`, 
        {headers: {"Authorization" : `Bearer ${loginUser.accessToken}`}})
        
        // 위 요청의 응답은 갱신되었거나 유효기간이 지나지 않은 원래 토큰이 담겨서 옵니다
...
const responseFail = (err)=>{
    console.log("respose fail error ----------")
    return Promise.reject(err);
}
